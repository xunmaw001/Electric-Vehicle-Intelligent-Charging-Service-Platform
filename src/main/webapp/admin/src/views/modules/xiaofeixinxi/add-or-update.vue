<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="消费编号" prop="xiaofeibianhao">
            <el-input v-model="ruleForm.xiaofeibianhao" 
                placeholder="消费编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.xiaofeibianhao" label="消费编号" prop="xiaofeibianhao">
              <el-input v-model="ruleForm.xiaofeibianhao" 
                placeholder="消费编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="发送日期" prop="fasongriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.fasongriqi" 
                type="date"
                :readonly="ro.fasongriqi"
                placeholder="发送日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.fasongriqi" label="发送日期" prop="fasongriqi">
              <el-input v-model="ruleForm.fasongriqi" 
                placeholder="发送日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="消费内容" prop="xiaofeineirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="消费内容"
                  v-model="ruleForm.xiaofeineirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xiaofeineirong" label="消费内容" prop="xiaofeineirong">
                    <span>{{ruleForm.xiaofeineirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.fasongriqi = this.getCurDate()
...
var objcross = this.$storage.getObj('crossObj');
      ...
      if(objcross!=null) {
	      objcross.jine = objcross.jine - this.ruleForm.jine
	      if(objcross.jine<0){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xiaofeixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xiaofeixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
